...
compras_lineas_raw = odoo.search_read(
    ...
    ['product_id', 'quantity', 'debit', 'credit', 'account_id', 'date', 'move_id', 'name', 'partner_id'],
    limit=100000
)
...
if compras_lineas:
    ...
    # Obtener proveedores (partners)
    partner_ids_compras = list(set([l.get('partner_id', [None])[0] for l in compras_lineas if l.get('partner_id')]))
    
    partners_compras_raw = odoo.models.execute_kw(
        odoo.db, odoo.uid, odoo.password,
        'res.partner', 'read',
        [partner_ids_compras, ['id', 'name', 'vat']]
    )
    
    partners_compras = partners_compras_raw if partners_compras_raw else []
    partner_map_compras = {p['id']: p for p in partners_compras}
    print(f"   - Proveedores únicos: {len(partner_map_compras)}")
...
for linea in compras_lineas:
    ...
    partner_id = linea.get('partner_id', [None])[0] if linea.get('partner_id') else None
    partner_info = partner_map_compras.get(partner_id, {}) if partner_id else {}
    partner_name = partner_info.get('name', 'Sin proveedor')
    partner_vat = partner_info.get('vat', '')
