...
- **Archivo principal:** `pages/6_📈_EstadoResultado.py` (autodetectado por `Home.py`).
- **Qué muestra:** métricas, tablas y gráficos del Estado de Resultado con comparaciones Real vs Presupuesto, mensualizado, YTD y tablero por cuenta contable.
- **API:** `/api/v1/estado-resultado/` (datos reales + centros) y `/api/v1/presupuesto/` (obtener y subir archivos). Los routers nuevos están en `backend/routers/estado_resultado.py` y `backend/routers/presupuesto.py`.
- **Filtros:** año, selección múltiple de meses y centro de costo habilitados desde la barra lateral.
- **Carga de archivos:** el botón lateral sube `BD_PPTO_<AÑO>.xlsx` a `data/`. Consulta `data/README.md` para saber cómo reemplazarlos manualmente si los descargas desde SharePoint.
- **Autenticación:** reutiliza `shared.auth.proteger_pagina()` para exigir inicio de sesión antes de mostrar el contenido.

## Panel de permisos

- **Archivo:** `pages/99_🛠️_Permisos.py`.
- **Descripción:** dashboard administrativo para asignar o revocar qué correos pueden ver dashboards restringidos.
- **Restricción:** solo disponible para los administradores definidos en `PERMISSION_ADMINS` (`mvalladares@riofuturo.cl`).
