...
# ========== FUNCIONES API ==========
def fetch_containers(start_date: str = None, end_date: str = None, 
...
def fetch_container_detail(sale_id: int) -> Dict:
...
with st.sidebar:
    ...
    # Botón de carga
    if st.button("🔄 Cargar Containers", type="primary", use_container_width=True):
    ...
    st.divider()
    st.caption("💡 Los containers se obtienen desde fabricaciones con PO asociada")
...
if st.session_state.get("load_containers", False) or "containers_data" not in st.session_state:
    with st.spinner("🔄 Cargando containers desde Odoo..."):
        containers = fetch_containers(state=state_options[selected_state])
...
with col_stats:
    ...
    for i, c in enumerate(containers_sorted[:5], 1):
...
if selected_key:
    ...
    # Información general del container
