...
def render(username: str, password: str):
    """Renderiza el contenido del tab Automatización Procesos."""
    ...
    if 'proc_pallets_list' not in st.session_state:
        st.session_state.proc_pallets_list = []
    if 'proc_orden_info' not in st.session_state:
        st.session_state.proc_orden_info = None
    if 'proc_last_result' not in st.session_state:
        st.session_state.proc_last_result = None
    
    # === MOSTRAR RESULTADO PERSISTENTE ===
    if st.session_state.proc_last_result:
        result = st.session_state.proc_last_result
        with st.container(border=True):
            ...
            if st.button("✖️ Cerrar mensaje", key="close_proc_result"):
                st.session_state.proc_last_result = None
                st.rerun()
        st.divider()
    ...
    with col_orden:
        orden_input = st.text_input(
            ...
            key="proc_orden_input"
        ).upper()
    
    with col_btn:
        st.write("")  # Espaciador
        st.write("")
        if st.button("🔍 Buscar", key="btn_buscar_orden", use_container_width=True):
            if orden_input:
                _buscar_orden(username, password, orden_input)
    
    # Mostrar info de orden encontrada
    if st.session_state.proc_orden_info:
        orden = st.session_state.proc_orden_info
        with st.container(border=True):
        ...
        def _convert_to_uppercase():
            if 'proc_pallets_input' in st.session_state:
                st.session_state.proc_pallets_input = st.session_state.proc_pallets_input.upper()
        
        pallets_textarea = st.text_area(
            "Ingresa los códigos de pallet (uno por línea)",
            placeholder="PACK0009900\nPACK0009901\nPACK0009902\n...",
            height=200,
            key="proc_pallets_input",
            on_change=_convert_to_uppercase,
            help="Escanea o pega los códigos de pallet. Puede ser PACK0009900 o PAC0009900"
        )
        
        if st.button("➕ Validar y Agregar", use_container_width=True, type="primary", key="btn_validar_pallets"):
            _procesar_pallets(username, password, pallets_textarea.upper(), tipo_mov)
        
        # === LISTA DE PALLETS VALIDADOS ===
        if st.session_state.proc_pallets_list:
            st.divider()
            st.subheader("4️⃣ Pallets a Agregar")
            
            # Resumen
            total_kg = sum(p['kg'] for p in st.session_state.proc_pallets_list)
            col_res1, col_res2 = st.columns(2)
            col_res1.metric("📦 Total Pallets", len(st.session_state.proc_pallets_list))
            col_res2.metric("⚖️ Total Kg", f"{total_kg:,.2f}")
            
            # Lista de pallets
            for idx, pallet in enumerate(st.session_state.proc_pallets_list):
                with st.container(border=True):
                    ...
                    with col_del:
                        if st.button("🗑️", key=f"del_pallet_{idx}", help="Eliminar pallet"):
                            st.session_state.proc_pallets_list.pop(idx)
                            st.rerun()
            ...
            with col_clear:
                if st.button("🗑️ Limpiar Lista", use_container_width=True, key="btn_clear_proc"):
                    st.session_state.proc_pallets_list = []
                    st.rerun()
            
            with col_submit:
                if st.button(f"✅ Agregar a {tipo_mov.upper()}", use_container_width=True, type="primary", key="btn_submit_proc"):
                    _agregar_a_orden(username, password, st.session_state.proc_orden_info['id'], tipo_mov)
    ...
    with st.spinner("Buscando orden..."):
        try:
            ...
            if orden_input.isdigit():
                # Solo número, buscar por ID directo
                search_term = orden_input
            elif '/' not in orden_input:
                # No tiene /, probablemente es solo el número
                search_term = orden_input
            else:
            ...
            if response.status_code == 200:
                data = response.json()
                if data.get('success'):
                    st.session_state.proc_orden_info = data['orden']
                    st.session_state.proc_pallets_list = []  # Limpiar pallets
                    st.rerun()
                else:
...
def _procesar_pallets(username: str, password: str, pallets_text: str, tipo_mov: str):
    ...
    codigos_existentes = {p['codigo'] for p in st.session_state.proc_pallets_list}
        ...
        try:
            response = requests.post(
                ...
                json={
                    ...
                    "tipo": tipo_mov,
                    "orden_id": st.session_state.proc_orden_info['id']
                },
                timeout=30
            )
            ...
            if response.status_code == 200:
                ...
                for pallet in data.get('pallets', []):
                    if pallet.get('valido'):
                        st.session_state.proc_pallets_list.append({
                ...
                # Limpiar textarea
                if 'proc_pallets_input' in st.session_state:
                    st.session_state.proc_pallets_input = ""
...
def _agregar_a_orden(username: str, password: str, orden_id: int, tipo_mov: str):
    ...
    pallets = st.session_state.proc_pallets_list
    ...
    with st.spinner(f"Agregando {len(pallets)} pallets a {tipo_mov}..."):
        try:
            ...
            if response.status_code == 200:
                ...
                st.session_state.proc_last_result = result
                
                if result.get('success'):
                    # Limpiar lista y actualizar orden
                    st.session_state.proc_pallets_list = []
                    # Refrescar info de orden
                    _buscar_orden(username, password, st.session_state.proc_orden_info['nombre'])
