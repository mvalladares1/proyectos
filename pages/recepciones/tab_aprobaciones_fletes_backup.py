...
def render_tabla_aprobaciones(username, password):
    ...
    if not ocs:
        st.info("✅ No hay órdenes de compra pendientes de aprobación de Máximo")
        return
    
    # PAGINACIÓN
    ITEMS_PER_PAGE = 20
    total_ocs = len(ocs)
    total_pages = (total_ocs + ITEMS_PER_PAGE - 1) // ITEMS_PER_PAGE
    
    if 'current_page' not in st.session_state:
        st.session_state.current_page = 1
    
    with col2:
        st.info(f"📦 {total_ocs} órdenes pendientes | Página {st.session_state.current_page} de {total_pages}")
    ...
    # Calcular rango de OCs a mostrar
    start_idx = (st.session_state.current_page - 1) * ITEMS_PER_PAGE
    end_idx = min(start_idx + ITEMS_PER_PAGE, total_ocs)
    ocs_pagina = ocs[start_idx:end_idx]
    
    # Calcular estadísticas de negociación (sobre todas las OCs, no solo la página)
    ...
    # Mostrar solo las OCs de la página actual
    # Convertir a DataFrame para mostrar
    df_data = []
    for oc in ocs_pagina:
        df_data.append({
            'ID': oc['oc_id'],
            'OC': oc['oc_name'],
            'Estado': oc['oc_state'],
    ...
    st.markdown("### Selecciona OCs para aprobar (página actual)")
    
    # Usar checkbox para selección masiva de la página
    if st.checkbox("Seleccionar todas de esta página", key="select_all_page"):
        selected_ids = [oc['oc_id'] for oc in ocs_pagina]
    else:
        selected_ids = []
    
    # Mostrar tabla con checkboxes individuales
    for idx, oc in enumerate(ocs_pagina):
        # Link directo a Odoo
        odoo_url = f"https://riofuturo.server98c6e.oerpondemand.net/web#id={oc['oc_id']}&model=purchase.order&view_type=form"
        # Link directo a Odoo
        odoo_url = f"https://riofuturo.server98c6e.oerpondemand.net/web#id={oc['oc_id']}&model=purchase.order&view_type=form"
        ...
        # Título del expander con link clicable
        titulo_expander = f"{'✅' if oc['tiene_info_logistica'] else '❌'} {emoji_negociacion} **{oc['oc_name']}** - {oc['proveedor']} - {fmt_dinero(oc['oc_amount'], decimales=0)}"
        ...
        with st.expander(titulo_expander):
            ...
            with col1:
                # Link directo a Odoo
                st.markdown(f"[🔗 Abrir en Odoo]({odoo_url})", unsafe_allow_html=True)
