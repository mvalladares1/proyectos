"""
Página de Etiquetas de Pallets
...
st.set_page_config(
    page_title="Etiquetas de Pallets",
    page_icon="🏷️",
    layout="wide"
)
...
def get_credentials():
    """Obtiene credenciales de sesión."""
...
st.title("🏷️ Generación de Etiquetas de Pallets")
...
if "pallets_cargados" not in st.session_state:
    st.session_state.pallets_cargados = []
if "cliente_nombre" not in st.session_state:
    st.session_state.cliente_nombre = ""

# ==================== PASO 1: SELECCIONAR CLIENTE ====================
st.header("1️⃣ Cliente")
...
with col1:
    ...
    if cliente_input:
        st.session_state.cliente_nombre = cliente_input

# ==================== PASO 2: BUSCAR ORDEN ====================
st.header("2️⃣ Buscar Orden de Producción")
    ...
    with st.spinner("Buscando órdenes..."):
        try:
            ...
            if ordenes:
                ...
                st.subheader("Órdenes encontradas:")
                ...
                for orden in ordenes:
                    ...
                    with col3:
                        if st.button("Seleccionar", key=f"sel_{orden.get('id')}", use_container_width=True):
                            st.session_state.orden_seleccionada = orden
                            st.rerun()
            else:
...
if st.session_state.orden_seleccionada:
    st.header("3️⃣ Pallets de la Orden")
    ...
    with col1:
        if st.button("🔄 Cargar Pallets", type="primary", use_container_width=True):
            with st.spinner("Obteniendo pallets..."):
                try:
                    pallets = obtener_pallets_orden(username, password, orden.get('name'))
                    st.session_state.pallets_cargados = pallets
    ...
    with col2:
        if st.button("❌ Cambiar Orden", use_container_width=True):
            st.session_state.orden_seleccionada = None
            st.session_state.pallets_cargados = []
            st.rerun()

# ==================== PASO 4: MOSTRAR PALLETS Y GENERAR ETIQUETAS ====================
if st.session_state.pallets_cargados:
    st.header("4️⃣ Etiquetas Disponibles")
    
    if not st.session_state.cliente_nombre:
        st.warning("⚠️ Por favor ingresa el nombre del cliente arriba")
    
    st.write(f"**Total de pallets:** {len(st.session_state.pallets_cargados)}")
    
    # Botón para generar todas las etiquetas
    if st.button("📥 Descargar Todas las Etiquetas (PDF)", type="primary"):
        st.info("🚧 Generación de PDF múltiple - Próximamente")
    
    st.divider()
    
    # Mostrar cada pallet
    for idx, pallet in enumerate(st.session_state.pallets_cargados, 1):
        with st.expander(f"**Pallet {idx}: {pallet.get('package_name', 'Sin nombre')}**", expanded=False):
            ...
            with col2:
                ...
                st.write(f"**Cantidad:** {pallet.get('qty_total', 0)}")
            ...
            if st.button(f"👁️ Vista Previa Etiqueta", key=f"prev_{idx}"):
                ...
                datos_etiqueta = {
                    ...
                    'cantidad_cajas': int(pallet.get('qty_total', 0)),
...
# ==================== FOOTER ====================
st.divider()
st.caption("🏷️ Sistema de Etiquetas de Pallets - Rio Futuro")
